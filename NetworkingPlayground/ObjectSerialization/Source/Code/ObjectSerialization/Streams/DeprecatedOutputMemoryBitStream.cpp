...
namespace Serialization { namespace Stream {
	...
	DeprecatedOutputMemoryBitStream::DeprecatedOutputMemoryBitStream() : mBuffer(nullptr), mBitHead(0), mBitCapacity(0)
	{
		ReallocBuffer(256);
	}

	DeprecatedOutputMemoryBitStream::~DeprecatedOutputMemoryBitStream()
	{
		std::free(mBuffer);
	}

	void DeprecatedOutputMemoryBitStream::WriteBitsInternal(uint8_t inData, size_t inBitCount)
	{
		assert(inBitCount <= 8);
	...
	void DeprecatedOutputMemoryBitStream::WriteBits(const void* inData, size_t inBitCount)
	{
	...
	void DeprecatedOutputMemoryBitStream::ReallocBuffer(uint32_t inNewBitLength)
	{
	...
	void DeprecatedOutputMemoryBitStream::WriteFreeBits(uint8_t inData, size_t inBitCount)
	{
	...
	void DeprecatedOutputMemoryBitStream::WriteBytesInternal(const void* inData, size_t inBitCount)
	{
